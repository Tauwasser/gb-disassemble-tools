...
class OpcodeXYZ(Opcode):
	def __init__(self, x, y, z, operation):
		Opcode.__init__(self, operation)
		self._bytes = bytearray([(x << 6) | (y << 3) | (z << 0)])
	
	def __str__(self):
		return "0x%02X (x%d y%d z%d)" % (self._bytes[-1], (self._bytes[-1] & 0xC0) >> 6, (self._bytes[-1] & 0x38) >> 3, (self._bytes[-1] & 0x07) >> 0)

class OpcodeXQPZ(Opcode):
	def __init__(self, x, q, p, z, operation):
		Opcode.__init__(self, operation)
		self._bytes = bytearray([(x << 6) | (p << 4) | (q << 3) | (z << 0)])
	
	def __str__(self):
		return "0x%02X (x%d q%d p%d z%d)" % (self._bytes[-1], (self._bytes[-1] & 0xC0) >> 6, (self._bytes[-1] & 0x30) >> 4, (self._bytes[-1] & 0x08) >> 3, (self._bytes[-1] & 0x07) >> 0)
	

class OpcodeCB_XYZ(OpcodeXYZ):
	def __init__(self, x, y, z, operation):
		OpcodeXYZ.__init__(self, x, y, z, operation)
...
z80gb_opcodes = [
	...
	# X3 Z7
	[OpcodeXYZ(3, y, 7, Operation('rst {0}', [], [Literal(y << 3, '${0:02X}')])) for y in range(0,8)],
]
...
for k, v in sorted(z80_opcode_dict.items()):
	f.write("0x{0:02X}: {1!s}\n".format(k, v.getOperation()))

for k, v in sorted(z80_CBOpcode_dict.items()):
